...
const ProjectView = () => {
  ...
  const [clipAnnotations, setClipAnnotations] = useState([
    ...
    useEffect(() => {
        ...
        const waitForLandmarks = async (currentProjectId, maxAttempts = 60, intervalMs = 2000) => {
                ...
                <div id="projectViewVideoPlayer">
                        {isLoading ? (
                            isProcessingLandmarks ? (
                                <p>Processing landmarks — this may take a minute. The project will open once processing finishes.</p>
                            ) : (
                                <p>Loading video...</p>
                            )
                        ) : (
                            <CustomVideoPlayer ref={videoPlayerRef} url={videoUrl} start={getCurrentStartClipTimeStamp()} end={getCurrentEndClipTimeStamp()} onPause={() => setVideoPaused(true)} onPlay={() => setVideoPaused(false)} landmarks={landmarks}></CustomVideoPlayer>
                        )}
