        ...
        if (nearest && nearest.landmarks) {
          ...
          if (intrinsicW > 0 && intrinsicH > 0) {
            const scale = Math.min(
              video.clientWidth / intrinsicW,
              video.clientHeight / intrinsicH
            );
          ...
          ctx.strokeStyle = "rgba(205, 124, 255, 0.95)";
          ctx.lineWidth = 1;
          let drewAny = false;
          for (const pair of POSE_CONNECTIONS) {
            ...
            const bIdx = pair[1];
            if (
              aIdx < nearest.landmarks.length &&
              bIdx < nearest.landmarks.length
            ) {
